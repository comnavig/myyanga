...
@section('title', 'Create Discover')
@section('page.title', 'Create Discover')
@section('content')
<div class="container mt-2">
	<div class="my-4">
		<h3 class="main-color">
			Form
			<a class="btn btn-sm main-btn-bg float-right" href="{{route('admin.discovers')}}">back to Discovers</a>
		</h3>
	</div>
	<div class="col-12">
		<form method="post" action="{{route('admin.discover.add')}}"  enctype="multipart/form-data" >
			@csrf
			<div class="row">
					...
					<div class="row">
						<div class="col-md-4 col-sm-12">
							<div class="form-group">
								...
								<select name="tag[name]" class="form-control" id="tag['name']" aria-describedby="tag_nameHelp" required >
									...
									<option value="Location" {{ ( old('tag.name') == 'Location' ? "selected" : "" )}}>Location</option>
									<option value="Occupation" {{ (old('tag.name') == 'Occupation' ? "selected" : "" )}}>Occupation</option>
								</select>
								@error('tag.name')
						...
						<div class="col-md-6 col-sm-12">
							<div class="form-group">
								...
								<input type="text" name="tag[value]" class="form-control" id="tag['value']" value="{{old('tag.value')}}" aria-describedby="tag_valueHelp" required />
								@error('tag.value')
