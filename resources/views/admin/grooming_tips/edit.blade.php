...
@section('title', 'Edit Grooming Tips')
@section('page.title', 'Edit Grooming Tips')
@section('content')
<div class="container mt-2">
	<div class="my-4">
		<h3 class="main-color">
			Form
			<a class="btn btn-sm main-btn-bg float-right" href="{{route('admin.groomingtips')}}">back to groomtips</a>
		</h3>
	</div>
	<div class="col-12">
		<form method="post" action="{{route('admin.groomingtip.update')}}"  enctype="multipart/form-data" >
			@csrf
			<input type="hidden" name="groomtips_id" value="{{$groomtips->id}}" />
			<div class="row">
				<div class="col-md-6 col-sm-12 mt-2">
					<div class="form-group">
						<label for="name">Name</label>
						<input type="text" name="name" class="form-control" id="name" value="{{$groomtips->name}}" aria-describedby="nameHelp" required />
		<!--
		...
		-->
					...
					@for ($i = 0; $i < 1; $i++)
						<div class="form-group">
							...
							<select name="categories[{{$i}}]" class="form-control" id="categories[{{$i}}]" aria-describedby="categoriesHelp" required >
								...
								<option value="{{$category->id}}" {{ ($groomtips->category_id == $category->id ? "selected" : "" )}}>{{$category->name}}</option>
								@endforeach	
							</select>
							<!--
				...
				<div class="col-md-12 col-sm-12 mt-2">
					<div class="form-group">
						...
						<textarea name="description" class="form-control" id="description" aria-describedby="descriptionHelp" rows="12" required >{{$groomtips->description}}</textarea>
						<small id="descriptionHelp" class="form-text text-muted">Please write groomtips content.</small>
						@error('description')
							...
							<div class="" style="width: 250px; height: 350px; overflow: hidden; padding: 20px; margin: 10px;">
									<label for="pictures[{{$i}}]">
										<div style="width: 250px; height: 250px; overflow: hidden; padding: 20px;">
											<img id="photo_img_{{$i}}" src="{{ $groomtips->picture[$i]->url }}" width="100%" />
										</div>
									</label>
